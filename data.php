<?php
    ...
    if ($_GET['GetUserInfo'] == 'true') {
        ...
        if ($_GET['AddInfo'] == 'true') {
            ...
            createMessageData($id, $account_id, $timestamp, $name, $picture, $text, $like_count);
            $result = mysqli_query($con, $sql);
...
function createMessageData($id, $account_id, $timestamp, $name, $picture, $text, $like_count)
{
    $sql = 'INSERT INTO `test_table` (id, account_id, timestamp, name, picture,
        text, like_count) VALUES (?, ?, ?, ?, ?, ?, ?)';
    $stmt = $GLOBALS['con']->prepare($sql);
    $stmt->bind_param('iiisssi', $id, $account_id, $timestamp, $name, $picture, $text, $like_count);
    $stmt->execute();
    $stmt->close();
}
